...
if (isset($mediaParcialF)) {
	...
	if ($mediaParcialF >= $media_aprovacao) {
		...
		//Verificar o id da fase nota do NumeroFase = 5
			$query = $pdo1->query("SELECT * FROM tbfasenota where IdPeriodo = '$periodo' and IdSerie = '$id_serie' and NumeroFase = 5");
			$res55 = $query->fetchAll(PDO::FETCH_ASSOC);

			$id_fase5 = $res55[0]['IdFaseNota'];

			$query = $pdo1->query("SELECT * FROM tbfasenotaaluno where IdTurma = '$turma' and IdAluno = '$aluno' and IdDisciplina = '$disciplina' and IdFaseNota = '$id_fase5'");
			$res = $query->fetchAll(PDO::FETCH_ASSOC);

			if (count($res) == 0) {

				$pdo1->query("INSERT INTO tbfasenotaaluno SET IdTurma = '$turma', IdDisciplina = '$disciplina', IdAluno = '$aluno', IdFaseNota = '$id_fase5'");
