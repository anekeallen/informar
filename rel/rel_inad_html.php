...
<table class='table' width='100%'  cellspacing='0' cellpadding='3'>
			...
			$query = $pdo->query("SELECT * FROM pgto_matriculas where data_vencimento >= '$dataInicial' and data_vencimento <= '$dataFinal' and pago != 'Sim' order by data_vencimento asc, id asc");
					...
					for ($i=0; $i < @count($res); $i++) { 
						...
						$query_usu = $pdo->query("SELECT * FROM tbaluno where IdAluno = '$aluno'");
						$res_usu = $query_usu->fetchAll(PDO::FETCH_ASSOC);
						$nome_aluno = $res_usu[0]['NomeAluno'];
