  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        $query = $pdo->query("SELECT * FROM tbaluno where IdAluno = '$_GET[id]' ");
        $res = $query->fetchAll(PDO::FETCH_ASSOC);
        $nome_aluno = $res[0]['NomeAluno'];


        ?>
        <h5 class="modal-title">Turmas do Aluno - <?php echo $nome_aluno ?></h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
            ...
            <div class="table-responsive">
              <table class="table table-bordered" id="dataTable_turmas" width="100%" cellspacing="0">
                ...
                <tbody>
                 ...
                 $query = $pdo->query("SELECT * FROM tbalunoturma where IdAluno = '$_GET[id]' order by IdAlunoTurma desc ");
                 $res = $query->fetchAll(PDO::FETCH_ASSOC);
                 ...
                 for ($i=0; $i < count($res); $i++) { 
                  ...
                  $Id_ST_aluno = $res[$i]['IdSituacaoAlunoTurma'];
                  $id_turma = $res[$i]['IdTurma'];
                  $id_m = $res[$i]['IdAlunoTurma'];

                  //$data = $res[$i]['data'];
                  //$dataF = implode('/', array_reverse(explode('-', $data)));


                  $query_r1 = $pdo->query("SELECT * FROM tbturma where IdTurma = '".$id_turma."' ");
                  $res_r1 = $query_r1->fetchAll(PDO::FETCH_ASSOC);

                  $id_serie = $res_r1[0]['IdSerie'];
                  $id_periodo = $res_r1[0]['IdPeriodo'];
                  $nome_turma = $res_r1[0]['NomeTurma'];
                  $sigla_turma = $res_r1[0]['SiglaTurma'];
                  $turno = $res_r1[0]['TurnoPrincipal'];
                  $dataInicial = $res_r1[0]['DataInicial'];
                  $dataFinal = $res_r1[0]['DataFinal'];
                  ...
                  $query_r2 = $pdo->query("SELECT * FROM tbperiodo where IdPeriodo = '".$id_periodo."' ");
                  $res_r2 = $query_r2->fetchAll(PDO::FETCH_ASSOC);
                  $ano = @$res_r2[0]['SiglaPeriodo'];

                  $query_r44 = $pdo->query("SELECT * FROM tbsituacaoalunoturma where IdSituacaoAlunoTurma = '$Id_ST_aluno' ");
